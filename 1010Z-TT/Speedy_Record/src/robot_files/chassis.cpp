...
void Chassis::driveControl()
{
  ...
  arcadeX = expDrive(master.get_analog(ANALOG_LEFT_X), cDriveExp, cJoyDead, cMotorMin);
	arcadeZ = expDrive(master.get_analog(ANALOG_RIGHT_X), cDriveExp, cJoyDead, cMotorMin);
  leftFront = arcadeY + arcadeX + arcadeZ;
	leftBack = arcadeY - arcadeX + arcadeZ;
  rightFront = arcadeY - arcadeX - arcadeZ;
	rightBack = arcadeY + arcadeX - arcadeZ;
...
void Chassis::autoStrafe(int direction, int target, int timeout, int speed){
	drive_PID.set_PID_vars(0.29, 0, 1.1, 0, 0);
	resetChassisSensors(true);
	drive_PID.target = target;
	startTimer(AUTO_DRIVE_TIMEOUT);
